...
def perform_ks_tests(run_path, max_steps=None):
    ...
    if not os.path.exists(checkpoint_path):
        print(f"No checkpoints found for run: {run_path}")
    ...
    for checkpoint_dir in sorted(
        [d for d in os.listdir(checkpoint_path) if d.isdigit()], key=int
    ):
        ...
        timestamp_dirs = [
            d
            for d in os.listdir(os.path.join(checkpoint_dir_path, latest_generated_dir))
            if d.startswith("20")
        ]
        ...
        plots_path = os.path.join(
            checkpoint_dir_path, latest_generated_dir, latest_timestamp_dir, "plots"
        )
    ...
    return checkpoint_numbers, p_values_by_key, ground_truth_data, predicted_data
...
def ks_test_model_checkpoints(run_path, training=False, max_steps=None):
    ...
    checkpoint_numbers, p_values_by_key, ground_truth_data, predicted_data = (
