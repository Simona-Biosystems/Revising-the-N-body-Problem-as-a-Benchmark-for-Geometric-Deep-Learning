...
class RandomFourierFeatures(torch.nn.Module):
    def __init__(self, out_dim, sigma, symmetric=None):
        ...
        if len(self.unconstraint_idx) > 0:
            self.frequencies_unconstraint = torch.stack(
                [
                    self.random_frequencies(self.sigma[i], self.num_frequencies)
                    for i in self.unconstraint_idx
                ],
                dim=0,
            )
        else:
        ...
        if len(self.constraint_idx) > 0:
            self.frequencies_constraint = torch.stack(
                [
                    self.random_frequencies(self.sigma[i], 2 * self.num_frequencies)
    ...
    def forward(self, x):
        ...
        if self.frequencies_unconstraint is not None:
            unconstraint_proj = x[
                ..., self.unconstraint_idx
            ] @ self.frequencies_unconstraint.type_as(x)
        ...
        if self.frequencies_constraint is not None:
            out = out * torch.einsum(
                "...d,di->...di",
                x[..., self.constraint_idx],
                self.frequencies_constraint.type_as(x),
            ).cos().prod(dim=-2)
