...
from models.ponita.geometry.invariants import (
    invariant_attr_r2s1_fiber_bundle,
    invariant_attr_r2s1_point_cloud,
    invariant_attr_r3s2_fiber_bundle,
    invariant_attr_r3s2_point_cloud,
    invariant_attr_rn,
)
        ...
        if graph.n == 2:
            ...
            if self.point_cloud:
                if graph.pos.size(-1) == graph.n:
                    ...
                    graph.attr = invariant_attr_r2s1_point_cloud(
                        graph.pos, graph.edge_index
                    )
                return graph
            else:
                if self.separable:
                    graph.attr, graph.fiber_attr = invariant_attr_r2s1_fiber_bundle(
                        graph.pos, graph.ori_grid, graph.edge_index, separable=True
                    )
                else:
                    graph.attr = invariant_attr_r2s1_fiber_bundle(
                        graph.pos, graph.ori_grid, graph.edge_index, separable=False
                    )
                return graph
        else:
            graph.dists = invariant_attr_rn(graph.pos[:, : graph.n], graph.edge_index)
            ...
            if self.point_cloud:
                if graph.pos.size(-1) == graph.n:
                    ...
                    graph.attr = invariant_attr_r3s2_point_cloud(
                        graph.pos, graph.edge_index
                    )
                return graph
            else:
                if self.separable:
                    graph.attr, graph.fiber_attr = invariant_attr_r3s2_fiber_bundle(
                        graph.pos, graph.ori_grid, graph.edge_index, separable=True
                    )
                else:
                    graph.attr = invariant_attr_r3s2_fiber_bundle(
                        graph.pos, graph.ori_grid, graph.edge_index, separable=False
                    )
                return graph
