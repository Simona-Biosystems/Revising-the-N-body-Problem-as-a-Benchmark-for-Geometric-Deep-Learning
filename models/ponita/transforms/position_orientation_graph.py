...
from models.ponita.geometry.rotation import uniform_grid_s2
from models.ponita.geometry.rotation_2d import uniform_grid_s1
from models.ponita.utils.to_from_sphere import scalar_to_sphere, vec_to_sphere
...
class PositionOrientationGraph(BaseTransform):
    ...
    def __call__(self, graph):
        ...
        if self.radius is not None:
            graph.edge_index = torch_geometric.nn.radius_graph(
                graph.pos[:, : graph.n],
                self.radius,
                graph.batch,
                loop,
    ...
    def to_po_fiber_bundle(self, graph):
        ...
        graph.ori_grid = (
            self.ori_grid_s1 if (graph.n == 2) else self.ori_grid_s2
        ).type_as(graph.pos)
        ...
        inputs = []
        if hasattr(graph, "x"):
            inputs.append(scalar_to_sphere(graph.x, graph.ori_grid))
        if hasattr(graph, "vec"):
            inputs.append(vec_to_sphere(graph.vec, graph.ori_grid))
        graph.x = torch.cat(
            inputs, dim=-1
        )  # [num_nodes, num_ori, input_dim + input_dim_vec]
    ...
    def to_po_point_cloud(self, graph):
        ...
        input_edge_index = remove_self_loops(
            coalesce(graph.edge_index, num_nodes=graph.num_nodes)
        )[0]
        ...
        graph.pos = torch.cat(
            [pos_t, ori_t], dim=-1
        )  # [4D, or 6D position-orientation element]
        ...
        num_base = pos.size(0)
        baseidx_source_liftidx = SparseTensor(
            row=target,
            col=source,
            value=lifted_index,
            sparse_sizes=(num_base, num_base),
        )

        # Determine the number of lifted_idx at each base node
        num_ori_at_base = (
            baseidx_source_liftidx.set_value(None).sum(dim=1).to(torch.long)
        )
        ...
        lifted_source = baseidx_source_liftidx[
            base_source
        ].storage.value()  # [648000 = 72000 * 9]

        # Then check the base nodes at the receiving end
        base_target = base_target.repeat_interleave(
            num_ori_at_base[base_source]
        )  # [64800]

        # Lookup all the lifted indices at the receiving (target) node
        lifted_target = baseidx_source_liftidx[
            base_target
        ].storage.value()  # [5832000 = 648000 * 9]
    ...
    def to_p_point_cloud(self, graph):
        ...
        graph.scatter_projection_index = torch.arange(0, graph.pos.size(0)).type_as(
            graph.batch
        )
